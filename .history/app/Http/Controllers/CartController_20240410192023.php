...
class CartController extends Controller
{
    ...
    //         }
    //         Session::put("cart",$cart);
    //         return redirect()->back()->with("message","Remove Success");
    //     }else{     
    //         return redirect()->back();
    //     }
    //  }
    public function deleteProductCart($session_id){
        ...
        if($cart == true){
            ...
            foreach($cart as $key =>  $val){
                if($val["session_id"] == $session_id){
                    unset($cart[$key]);
                    // Kiểm tra nếu mảng rỗng sau khi xóa
                    if(empty($cart)){
                        Session::forget("cart");
                    } else {
                        Session::put("cart", $cart);
     ...
     public function deleteProductCartAll(){
        $cart = Session::get("cart");
        if($cart==true){
            // Session::destroy();
            Session::forget("cart");
            ...
            return redirect()->back()->with("message","Remove all product  success");
        }
     }
     //COUPON
