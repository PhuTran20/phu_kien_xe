                        ...
                        <table  class="table table-striped table-bordered"> 
                            ...
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <th class="name class="image-prod"">Image</th>
                            ...
                            <tbody>        
                                ...
                                <tr>
                                    ...
                                    <td class="image-prod" ><img   style=" border-radius: 0;width: 100%;height:100%;" src="{{ URL::to('/frontend/images/'.$item->image)}}" alt=""></td>
                                    <td class="name">{{$item->quantity}}</td>
                             
                                    <td class="name">
                                        @php
                                            $bike_names = []; // Biến trung gian để lưu danh sách tên xe
                                            $id_bikes = explode(',', $item->id_bike); // Phân tách chuỗi thành mảng các id_bike
                                            foreach ($id_bikes as $id_bike) {
                                                foreach ($bike as $data) {
                                                    if ($id_bike == $data->id_bike) {
                                                        $bike_names[] = $data->name_bike; // Thêm tên xe vào danh sách
                                                    }
                                                }
                                            }
                                            echo implode('<br>', $bike_names); // Hiển thị danh sách tên xe
                                        @endphp
                                    </td>
